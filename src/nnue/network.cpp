...
struct EmbeddedNNUE {
  EmbeddedNNUE(const unsigned char *embeddedData,
      ...
      : data(embeddedData), end(embeddedEnd), size(embeddedSize) {}
...
template <typename Arch, typename Transformer>
Value Network<Arch, Transformer>::evaluate(const Position &pos, bool adjusted,
                                           int *complexity,
                                           bool psqtOnly) const {
  ...
  TransformedFeatureType transformedFeaturesUnaligned
      [FeatureTransformer<Arch::TransformedFeatureDimensions,
                          nullptr>::BufferSize +
...
#else
  alignas(alignment)
      TransformedFeatureType transformedFeatures[FeatureTransformer<
          Arch::TransformedFeatureDimensions, nullptr>::BufferSize];
#endif
  ...
  const auto psqt =
      featureTransformer->transform(pos, transformedFeatures, bucket, psqtOnly);
  const auto positional =
      !psqtOnly ? (network[bucket]->propagate(transformedFeatures)) : 0;
...
void Network<Arch, Transformer>::verify(std::string evalfilePath) const {
  ...
  if (evalFile.current != evalfilePath) {
    std::string msg1 = "Network evaluation parameters compatible with the "
                       "engine must be available.";
    std::string msg2 =
        "The network file " + evalfilePath + " was not loaded successfully.";
    std::string msg3 =
...
template <typename Arch, typename Transformer>
NnueEvalTrace
Network<Arch, Transformer>::trace_evaluate(const Position &pos) const {
  // We manually align the arrays on the stack because with gcc < 9.3
  // overaligning stack variables with alignas() doesn't work correctly.
  constexpr uint64_t alignment = CacheLineSize;
...
template <typename Arch, typename Transformer>
void Network<Arch, Transformer>::load_user_net(
    const std::string &dir, const std::string &evalfilePath) {
...
Network<Arch, Transformer>::load(std::istream &stream) {
  initialize();
...
template <typename Arch, typename Transformer>
bool Network<Arch, Transformer>::read_parameters(
    std::istream &stream, std::string &netDescription) const {
  std::uint32_t hashValue;
